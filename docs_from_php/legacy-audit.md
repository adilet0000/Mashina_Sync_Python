...
| `Dealer/User` API response | `BaseSync::getUser()` | Old API user/dealer, no local table | `dealer.status`, `phones` | Owns ads; supplies contacts | Every sync command validates active dealer | In catalog mode this is replaced by env `SYNC_CATALOG_USER_ID_*` and `SYNC_CATALOG_PHONES_*`. |
